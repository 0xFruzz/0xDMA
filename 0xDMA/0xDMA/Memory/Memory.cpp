...
bool Memory::DumpMemoryMap(bool debug)
{
	...
	if (!handle)
	{
		LOG("[0xGOIDA] Failed to open a VMM Handle\n");
		return false;
	}
	...
	{
		LOG("[0xGOIDA] Failed to get physical memory map\n");
		VMMDLL_Close(handle);
		return false;
	}
	...
	{
		LOG("[0xGOIDA] Invalid VMM Map Version\n");
	...
	{
		printf("[0xGOIDA] Failed to get physical memory map\n");
		VMMDLL_MemFree(pPhysMemMap);
		VMMDLL_Close(handle);
		return false;
	}
...
bool Memory::SetFPGA()
{
	...
	{
		LOG("[0xGOIDA] Failed to lookup FPGA device, Attempting to proceed\n\n");
		return false;
	}
	...
	if ((qwVersionMajor >= 4) && ((qwVersionMajor >= 5) || (qwVersionMinor >= 7)))
	{
		...
		if (!handle)
		{
			LOG("[0xGOIDA] Failed to create FPGA device\n");
			return false;
		}
...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	...
	reinit:
		...
		if (memMap)
		{
			...
			if (!std::filesystem::exists(path))
				...
				dumped = true;
			LOG("[0xGOIDA] Начал дампить память в файл...\n");
			if (!dumped)
			{
				LOG("[0xGOIDA] Ай залупа, не смог сдампить!\n");
			}
			else
			{
				LOG("[0xGOIDA] Ураа, сдампили\n");
		...
		if (!this->vHandle)
		{
			...
			LOG("[0xGOIDA] Не смог инициализировать, ты точно используешь DMA карту? Проверь подключена ли она.\n");
			return false;
		}
		...
		LOG("[0xGOIDA] FPGA ID: %llu\n", FPGA_ID);
		LOG("[0xGOIDA] DEVICE ID: %llu\n", DEVICE_ID);

		if (!this->SetFPGA())
		{
			LOG("[0xGOIDA] Could not set FPGA!\n");
			VMMDLL_Close(this->vHandle);
			return false;
		}
		...
		LOG("[0xGOIDA] DMA уже инициализирована.\n");

	if (PROCESS_INITIALIZED)
	{
		LOG("[0xGOIDA] Процесс игры уже инициализирован\n");
		return true;
	}
	...
	{
		LOG("[0xGOIDA] Не смог получить PID игры, ты точно раст запустил блять?\n");
		return false;
	}
	this->current_process.process_name = process_name;
	if (!TargetProcess.FixCr3())
		std::cout << "[0xGOIDA] Не получилось починить первоначальное CR3, возможно не попал в тайминг EAC" << std::endl;
	else
		std::cout << "[0xGOIDA] CR3 Работает" << std::endl;

	this->current_process.base_address = GetBaseAddress(process_name);
	if (!this->current_process.base_address)
	{
		LOG("[0xGOIDA] Плачем, BaseAddress не получен!\n");
		return false;
	}
	...
	{
		LOG("[0xGOIDA] Плачем, BaseSizee не получен!\n");
		return false;
	}

	LOG("[0xGOIDA] Информация о %s\n", process_name.c_str());
	LOG("[0xGOIDA] PID: %i\n", this->current_process.PID);
	LOG("[0xGOIDA] Base Address: 0x%p\n", this->current_process.base_address);
	LOG("[0xGOIDA] Base Size: 0x%p\n", this->current_process.base_size);
...
std::vector<std::string> Memory::GetModuleList(std::string process_name)
{
	...
	{
		LOG("[0xGOIDA] Failed to get module list\n");
		return list;
	}
...
VMMDLL_PROCESS_INFORMATION Memory::GetProcessInformation()
{
	...
	{
		LOG("[0xGOIDA] Failed to find process information\n");
		return { };
	}
...
size_t Memory::GetBaseAddress(std::string module_name)
{
	...
	if (!Modules.contains(str))
	{
		...
		{
			LOG("[0xGOIDA] Couldn't find Base Address for %s\n", module_name.c_str());
			return 0;
		}
...
uintptr_t Memory::GetExportTableAddress(std::string import, std::string process, std::string module)
{
	...
	if (!result)
	{
		LOG("[0xGOIDA] Failed to get Export Table\n");
		return 0;
	}
...
uintptr_t Memory::GetImportTableAddress(std::string import, std::string process, std::string module)
{
	...
	if (!result)
	{
		LOG("[0xGOIDA] Failed to get Import Table\n");
		return 0;
	}
...
{
	LOG("[0xGOIDA] Memory dumping currently does not rebuild the IAT table, imports will be missing from the dump.\n");
	...
	for (size_t i = 0; i < nt.FileHeader.NumberOfSections; i++, sections++)
	{
		...
		LOG("[0xGOIDA] Rewriting file offsets at 0x%p size 0x%p\n", sections->VirtualAddress, sections->Misc.VirtualSize);
	...
	//LOG("[0xGOIDA] Creating new import section\n");
...
bool Memory::Write(uintptr_t address, void* buffer, size_t size) const
{
	...
	{
		LOG("[0xGOIDA] Failed to write Memory at 0x%p\n", address);
		return false;
	}
	return true;
}
...
bool Memory::Read(uintptr_t address, void* buffer, size_t size) const
{
	...
	{
		LOG("[0xGOIDA] Failed to read Memory at 0x%p\n", address);
		return false;
	}
	return true;
}
...
VMMDLL_SCATTER_HANDLE Memory::CreateScatterHandle()
{
	...
	if (!ScatterHandle)
		LOG("[0xGOIDA] Failed to create scatter handle\n");
...
void Memory::AddScatterReadRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size)
{
	...
	{
		LOG("[0xGOIDA] Failed to prepare scatter read at 0x%p\n", address);
	}
}
...
void Memory::AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size)
{
	...
	{
		LOG("[0xGOIDA] Failed to prepare scatter write at 0x%p\n", address);
	}
}
